...
from backend.filesystem import CardModel, CardApplication
from backend.cards import card_detect, SimCardBase, UiccCardBase
from backend.runtime import RuntimeState
from backend.profile import CardProfile
from backend.cdma_ruim import CardProfileRUIM
from backend.ts_102_221 import CardProfileUICC
...
import backend.ts_31_102
import backend.ts_31_103
import backend.ts_31_104
import backend.ara_m
import backend.global_platform
import backend.euicc
...
def init_card(sl: LinkBase) -> Tuple[RuntimeState, SimCardBase]:
    ...
    state object (rs) is required for all backend-shell commands.
    """
    ...
    if profile is None:
        ...
        # just means that backend was unable to recognize the card profile. This
    ...
    # We cannot do it within backend/profile.py as that would create circular
    ...
    isd_r = rs.mf.applications.get(backend.euicc.AID_ISD_R.lower(), None)
    if isd_r:
        rs.lchan[0].select_file(isd_r)
        try:
            rs.identity['EID'] = backend.euicc.CardApplicationISDR.get_eid(scc)
        except SwMatchError:
