...
from backend.utils import Hexstr, SwHexstr, SwMatchstr
...
backend.global_platform.DataCollection.possible_nested.append(Sgp02Eid)
...
# GlobalPlatform 2.1.1 Section 9.9.3.1 from backend/global_platform.py extended with E0
class FciTemplate(BER_TLV_IE, tag=0x6f, nested=backend.global_platform.FciTemplateNestedList +
...
class LoadCRL(BER_TLV_IE, tag=0xbf35, nested=[]): 
    pass

# SGP.22 Section 5.7.15: GetProfilesInfo
class TagList(BER_TLV_IE, tag=0x5c):
    _construct = GreedyRange(Int8ub) 
class ProfileInfoListReq(BER_TLV_IE, tag=0xbf2d, nested=[TagList]): 
    pass
...
class RefreshFlag(BER_TLV_IE, tag=0x81): 
    _construct = Int8ub 
...
class CardApplicationISDR(backend.global_platform.CardApplicationSD):
    def __init__(self):
...
class CardApplicationECASD(backend.global_platform.CardApplicationSD):
    def decode_select_response(self, data_hex: Hexstr) -> object:
        t = FciTemplate()
        t.from_tlv(h2b(data_hex))
        d = t.to_dict()
