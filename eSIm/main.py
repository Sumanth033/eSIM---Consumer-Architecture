...
import backend.esim.rsp as rsp
from backend.esim import saip, PMO
from backend.esim.es8p import *
from backend.esim.x509_cert import oid, cert_policy_has_oid, cert_get_auth_key_id
from backend.esim.x509_cert import CertAndPrivkey, CertificateSet, cert_get_subject_key_id, VerifyError
...
class SmDppHttpServer:
    ...
    def initiateAutentication(self, request: IRequest, content: dict) -> dict:
        ...
        output['serverCertificate'] = b64encode2str(self.dp_auth.get_cert_as_der()) 
    ...
    def authenticateClient(self, request: IRequest, content: dict) -> dict:
        ...
        print("matching id over")
        # Put together profileMetadata + _bin
        ss.profileMetadata = ProfileMetadata(iccid_bin=h2b(swap_nibbles(iccid_str)), spn="Nameesim", profile_name=matchingId)
